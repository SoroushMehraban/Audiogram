...
import com.reglardo.audiogram.TimerService
import com.reglardo.audiogram.adapter.TagAdapter
import com.reglardo.audiogram.databinding.FragmentRecorderBinding
import java.io.File
...
class RecorderFragment : Fragment() {
        ...
        binding.saveTrim.setOnClickListener {
            ...
            if (trimOutput == "") {
                ...
                if (file.exists()) {
                    showAlertDialog(
                        "Error",
                        "Recording with the same name exists. Try another one."
                    )
                } else {
                    val startTime =
                        (binding.trimSeekBar.selectedMinValue.toDouble() / 1000).toString()
    ...
    private fun handleFileIntentIfExists() {
        val selectedFile = activity?.intent?.extras?.getString(FILE)
        val previousState = activity?.intent?.extras?.getString(RecordingListActivity.FROM).toString()
    ...
    private fun enableTrimSeekBar() {
        ...
        binding.trimEnd.text =
            getTimeStringFromDouble((mediaViewModel.mediaPlayer.duration / 1000).toDouble())
    ...
    private fun changePitchConstraints(state: String) {
        if (state == "afterPlay") {
            ...
            val normalPitchParams =
                binding.normalPitchBtn.layoutParams as ConstraintLayout.LayoutParams
